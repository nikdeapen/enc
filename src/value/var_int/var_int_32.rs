...
impl VarInt32 {
    ...
    /// Creates a `VarInt32` from the signed `value` using zigzag encoding.
    pub fn from_zigzag(value: i32) -> Self {
        let value: u32 = value as u32;
        let value: u32 = (value << 1) ^ (value >> 31);
        Self::from(value)
    }

    /// Creates a `VarInt32` from the signed `value` using zigzag encoding.
    pub fn to_zigzag(&self) -> i32 {
