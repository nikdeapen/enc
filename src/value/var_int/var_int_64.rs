...
use crate::{read_single_byte, StreamError};
use crate::{DecodeFromReadPrefix, EncodeToSlice, EncodedLen, Error};
...
impl VarInt64 {
    ...
    /// Creates a `VarInt64` from the `i64` value.
    pub fn from_i64(value: i64) -> Self {
        Self::from(((value << 1) ^ (value >> 63)) as u64)
    }

    /// Converts the `u64` value to a `VarInt64`.
...
mod tests {
    ...
    fn decode() -> Result<(), io::Error> {
        for (expected, input) in TEST_CASES {
            ...
            let result: Result<VarInt64, StreamError> =
