        ...
        impl $crate::EncodedLen for $target_type {
            fn encoded_len(&self) -> Result<usize, Error> {
                let encoded_len: usize = if self.value == 0 {
                    ...
                    let bits: u32 = $bit_size - self.value.leading_zeros();
...
mod tests {
    ...
    fn var_int_16() {
        ...
        for (value, encoded) in test_cases {
            test_io(&VarInt16::from(value), encoded, false);
        }
    }
    ...
    fn var_int_32() {
        ...
        for (value, encoded) in test_cases {
            test_io(&VarInt32::from(value), encoded, false);
        }
    }
    ...
    fn var_int_64() {
        ...
        for (value, encoded) in test_cases {
            test_io(&VarInt64::from(value), encoded, false);
        }
    }
    ...
    fn var_int_128() {
        ...
        for (value, encoded) in test_cases {
            test_io(&VarInt128::from(value), encoded, false);
        }
    }
    ...
    fn var_int_size() {
        ...
        for (value, encoded) in test_cases {
            test_io(&VarIntSize::from(value), encoded, false);
        }
    }
}
