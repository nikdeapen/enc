...
impl SpecialSet {
    ...
    /// Adds the special char. If the char is invalid or already present this has no effect.
    pub fn add(&mut self, c: u8) {
        self.bits |= 1u32.checked_shl(Self::index_of(c) as u32).unwrap_or(0);
    }

    /// Removes the special char. If the char is invalid or not present this has no effect.
...
#[cfg(test)]
mod tests {
    use crate::percent::special_set::SpecialSet;
